...
from llm_reliability_analytics.oracles import evaluate_with_oracle
...
def run_batch_workflow(
    ...
    run = create_test_run(name=run_name, model_name=model_name)

    runner = TestRunner(llm_client=MockLLMClient(mode=mode, seed=seed))
    results = runner.run(test_cases, run_id=run.id)
...
def score_results_with_oracles(test_cases: list[TestCase], results: list[TestResult]) -> None:
    ...
    for test_case, result in zip(test_cases, results, strict=True):
        ...
        # For demo datasets, semantic/custom cases can reuse keyword matching.
        if oracle_type == "keyword_match" and "keywords" not in metadata:
            metadata["keywords"] = [
                part.strip() for part in test_case.expected_answer.split(",") if part.strip()
            ]
            metadata.setdefault("mode", "any")
